...
def foreign_objects_ajax(request):
   ...
   # Получаем параметры запроса
    ...
    objects = foreign_filter.filterByRoomAmount(objects, params, 5)
    objects = foreign_filter.filterByFeatures(objects, params)    
    ...
    objects = foreign_filter.filterByFieldWithRange(objects, params, 'area_object', 'area_object')
    objects = foreign_filter.filterByFieldWithRange(objects, params, 'price_sell_u', 'price_sell_u')
    objects = foreign_filter.filterByFieldWithRange(objects, params, 'price_rent_monthly', 'price_rent_monthly')
    ...
    # Cортировка
    if params['purpose'] == 'r':
    	objects = foreign_filter.sortRent(objects, params)
    else:
    	objects = foreign_filter.sortBuy(objects, params)
...
def buy(request):
    ...
    ids = json.dumps(Object.objects.filter(purpose__in=['s', 'sr']).values('id'), cls=render_to_json.ObjectsJSONEncoder)
    return locals()
...
def rent(request):
    ...
    countries = json.dumps(Object.objects.values('country').distinct(), cls=render_to_json.ObjectsJSONEncoder)
    ids = json.dumps(Object.objects.filter(purpose__in=['r', 'sr']).values('id'), cls=render_to_json.ObjectsJSONEncoder)
    return locals()
...
def object(request, requestType, objectId):
    ...
    # Цена Аренды
    if object.price_rent_monthly:
    	object.price_rent_monthly = "{:,} $".format(object.price_rent_monthly).replace(',', ' ')
